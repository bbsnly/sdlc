...
func TestACommitInAnotherRepositoryIsNotTheStorys(t *testing.T) {
	...
	refused(t, `[System.Environment]::SetEnvironmentVariable( "GIT_DIR", "/work/project/.git"); git commit -am x`, psElsewhere, "commit-gate")
	allowed(t, `grep -rn "GIT_DIR" . ; git commit -m x`, elsewhere)
	allowed(t, `git commit -m "hooks: stop reading 'GIT_DIR'"`, elsewhere)
...
func TestEveryCommandThatCommitsMeetsTheGate(t *testing.T) {
	for _, command := range []string{
		...
		"git merge -m --no-commit wip",
		"git merge --no-commit --commit wip",
		"git merge -n wip",
		"git commit-tree --no-commit 4b825dc",
		"git rebase --no-commit main",
		"git commit --squash HEAD -am x",
	...
	for _, command := range []string{
		"git merge --abort",
		"git rebase --quit",
		"git cherry-pick -n 1a2b3c",
		"git revert --no-commit HEAD",
		"git merge --squash wip",
		"git merge --no-ff --no-commit wip",
