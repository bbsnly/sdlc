...
# `irm ... | iex`, the way this is usually run, evaluates the script in the
# caller's own scope. Without a scope of its own, the preferences and
# variables below stayed set in the user's PowerShell session after it
# finished: every error there that would have let a command carry on
# stopped it instead. $env:Path belongs to the process, so the PATH update
# at the end still reaches that session.
& {
